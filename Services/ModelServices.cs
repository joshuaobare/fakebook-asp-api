...
public class UserService
{
    ...
    public async Task ReplaceAsync(string id, Users updatedUser) =>
        await _userCollection.ReplaceOneAsync(x => x.UserId == id, updatedUser);

    public async Task RemoveAsync(string id) =>
        await _userCollection.DeleteOneAsync(x => x.UserId == id);

    public async Task UpdateAsync(FilterDefinition<Users>filter, UpdateDefinition<Users> update) => await _userCollection.UpdateOneAsync(filter, update);
